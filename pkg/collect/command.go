...
func CreateCollectCmd() *cobra.Command {
	...
	cmd.Flags().StringVar(&cc.kubeconfig, "kubeconfig", "", "kubeconfig for shoot cluster, uses KUBECONFIG if not specified.")
...
func (cc *collectCommand) setupClientSet() error {
	if cc.kubeconfig == "" {
		cc.kubeconfig = os.Getenv("KUBECONFIG")
	}
	if cc.kubeconfig == "" {
		if home := homedir.HomeDir(); home != "" {
			cc.kubeconfig = filepath.Join(home, ".kube", "config")
		}
	}
	if cc.kubeconfig == "" {
	...
	if err != nil {
		return fmt.Errorf("error on config from kubeconfig file %s: %s", cc.kubeconfig, err)
	}
